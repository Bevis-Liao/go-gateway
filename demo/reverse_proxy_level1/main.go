...
func main() {
	nextProxyAddr := "http://127.0.0.1:2002"
	nextProxyUrl, err := url.Parse(nextProxyAddr)
	if err != nil {
		log.Println(err)
	}
	urls := []*url.URL{nextProxyUrl}
	// 获取 reverse proxy 实体，实体实现 serverHttp
	proxyHandler := NewMultipleHostsReverseProxy(urls)
	// 创建 http 服务器
